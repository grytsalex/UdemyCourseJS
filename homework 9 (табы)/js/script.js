...
window.addEventListener('DOMContentLoaded', function() {    // событие срабатывает после прогрузки DOM дерева (все теги)
    'use strict';
    ...
    function hideTabContent(a) {                            // функция скрывет наши табы принимает один технический аргумент
    ...
    function showTabContent(b) {  // функция показывает 1таб, передаем техн. параметр который позволяет показать именно тот tab-content который нам нужен
        if (tabContent[b].classList.contains('hide')){  // проверяем действительно ли этот елемент скрыт
   ...
   info.addEventListener('click', function(event){  // используем делегирование, помещаем обьект event для сравнениея с тем куда мы кликаем
        let target = event.target;  // создаем переменную для удобства     

        if (target && target.classList.contains('info-header-tab')) {  // если мы кликаем четко в 'info-header-tab' то выполняем следующее
            for (let i = 0; i < tab.length; i++) { // перебираем все табы
                if (target == tab[i]) {  // сравниваем табы с тем куда мы кликнули, и если совпадет то мы выполняем действие
